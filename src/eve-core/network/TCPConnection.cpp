...
bool TCPConnection::SendData( char* errbuf )
{
    ...
    while( !mSendQueue.empty() )
    {
        ...
        int status = mSock->send( &(*buf)[ 0 ], (uint)buf->size(), MSG_NOSIGNAL );
...
bool TCPConnection::RecvData( char* errbuf )
{
    ...
    while( true )
    {
        ...
        int status = mSock->recv( &(*mRecvBuf)[ 0 ], (uint)mRecvBuf->size(), 0 );
...
void TCPConnection::TCPConnectionLoop()
{
    ...
    sLog.Debug( "        Threading", "Starting TCPConnectionLoop with thread ID 0x%X", pthread_self() );
#endif /* !HAVE_WINDOWS_H */
...
#ifndef HAVE_WINDOWS_H
    sLog.Debug( "       Threading", "Ending TCPConnectionLoop with thread ID 0x%X", pthread_self() );
#endif /* !HAVE_WINDOWS_H */
}
