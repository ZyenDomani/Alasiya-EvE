...
bool ClassDecodeGenerator::ProcessInt(const TiXmlElement* field)
{
    ...
    /** @note:  commented code is depreciated in favor of PyRep::IntegerValue(), which tests and decodes all number Py*types as integers
...
bool ClassDecodeGenerator::ProcessBool(const TiXmlElement* field)
{
    ...
    const char* v = top();
    fprintf(mOutputFile, "    %s = (bool)PyRep::IntegerValue(%s);\n", name, v);
...
bool ClassDecodeGenerator::ProcessNone(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
            "    if (!%s->IsNone()) {\n"
            "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is not a PyNone: %%s\", %s->TypeString());\n"
            "        /** @todo  should there be more here?  if this *should* be None, do we need to find out what/why?*/\n"
            "        return false;\n"
            "    }\n\n",
            v, mName, v, v
   );
...
bool ClassDecodeGenerator::ProcessString(const TiXmlElement* field)
{
    ...
    if (none_marker != nullptr) {
        fprintf(mOutputFile,
            "    if (%s->IsNone()) {\n"
            "        %s = \"%s\";\n"
            "    } else if (%s->IsWString() or %s->IsString()) {\n"
            "        %s = PyRep::StringContent(%s);\n"
            "    } else {\n"
...
bool ClassDecodeGenerator::ProcessToken(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "        %s = %s->AsToken();\n"
        "    } else {\n"
        "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is neither None nor a token: %%s\", %s->TypeString());\n"
        "        return false;\n"
        "    }\n\n",
        v,
            name,
...
bool ClassDecodeGenerator::ProcessTuple(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "        %s = %s->AsTuple();\n"
        "    } else {\n"
        "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is neither None nor a tuple: %%s\", %s->TypeString());\n"
        "        return false;\n"
        "    }\n\n",
        v,
            name,
...
bool ClassDecodeGenerator::ProcessList(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "        %s = %s->AsList();\n"
        "    } else {\n"
        "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is neither None nor a list: %%s\", %s->TypeString());\n"
        "        return false;\n"
        "    }\n\n",
        v,
            name,
...
{
    //first, we need to know how many elements this list has:
    const TiXmlNode* i(nullptr);
...
bool ClassDecodeGenerator::ProcessListInt(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "            return false;\n"
        "        }\n\n"
        "	 //1 i think this is using copy c'tor, as move c'tors and both assigns are disabled.\n"
...
bool ClassDecodeGenerator::ProcessListStr(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "            return false;\n"
        "        }\n\n"
        "	 //3 i think this is using copy c'tor, as move c'tors and both assigns are disabled.\n"
        "	 //  if thats the case (and i think it is), this copy will need to be decref'd\n"
...
bool ClassDecodeGenerator::ProcessDict(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "        %s = %s->AsDict();\n"
        "    } else {\n"
        "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is neither None nor a dict: %%s\", %s->TypeString());\n"
        "        return false;\n"
        "    }\n\n",
        v,
            name,
...
bool ClassDecodeGenerator::ProcessDictInline(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        "    if (!%s->IsDict()) {\n"
        "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is not PyDict: %%s\", %s->TypeString());\n"
        "        return false;\n"
        "    }\n\n"
    ...
    while ((i = field->IterateChildren(i))) {
        if (i->Type() == TiXmlNode::TINYXML_ELEMENT) {
            ...
            fprintf(mOutputFile, "    bool %s_%u = false;\n", iname, count);
            ++count;
        }
    }
    ...
    if (count < 1) {
        ...
        if (soft) {
            ...
            fprintf(mOutputFile,
                " {\n"
                "            _log(XMLP__DECODE_ERROR, \"Decode %s failed: Unknown key string '%%s' in %s\", key_string__->content().c_str());\n"
                "            // is there something to clean up here?? \n"
                "            return false;\n"
                "        }\n"
                "    }\n\n",
                mName, iname
...
bool ClassDecodeGenerator::ProcessDictRaw(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "	 //4 i think this is using copy c'tor, as move c'tors and both assigns are disabled.\n"
        "	 //  if thats the case (and i think it is), this copy will need to be decref'd\n"
        "        if (!%s_cur->second->Is%s()) {\n"
        "            _log(XMLP__DECODE_ERROR, \"Decode %s failed: Value %%u in dict %s is not %s: %%s\", %s_index, %s_cur->second->TypeString());\n"
        "            PyDecRef(%s);\n"
        "            PyDecRef(k);\n"
        "            return false;\n"
        "        }\n\n"
        "        const Py%s *v = %s_cur->second->As%s();\n"
        "	 //5 i think this is using copy c'tor, as move c'tors and both assigns are disabled.\n"
        "	 //  if thats the case (and i think it is), this copy will need to be decref'd\n"
        "        %s[ k->value() ] = v->value();\n"
        "    }\n\n",
        v,
...
bool ClassDecodeGenerator::ProcessDictInt(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        "    if (!%s->IsDict()) {\n"
        "        _log(XMLP__DECODE_ERROR, \"Decode %s failed: %s is not a dict: %%s\", %s->TypeString());\n"
        "        // is there something to clean up here??  yes, but im not sure how to get it yet...\n"
        "        return false;\n"
        "    }\n\n"
        "    const PyDict* %s(%s->AsDict());\n"
        "    PyDict::const_iterator %s_cur = %s->begin(), %s_end = %s->end();\n"
        ...
        "        %s[ k->value() ] = %s_cur->second;         // removed Clone() here\n"
        "	 //6 i think this is using copy c'tor, as move c'tors and both assigns are disabled.\n"
        "	 //  if thats the case (and i think it is), this copy will need to be decref'd\n"
        "    }\n\n",
        v,
            mName, v, v,
...
bool ClassDecodeGenerator::ProcessDictStr(const TiXmlElement* field)
{
    ...
    fprintf(mOutputFile,
        ...
        "        %s[ k->content() ] = %s_cur->second;          // removed Clone() here\n"
        "	 //7 i think this is using copy c'tor, as move c'tors and both assigns are disabled.\n"
        "	 //  if thats the case (and i think it is), this copy will need to be decref'd\n"
        "    }\n\n",
        v,
            mName, v, v,
