...
"/*  Alasiya-EvE: EVE Online Server Emulator\n"
"  \n"
"  **************************************************************\n"
...
void XMLPacketGen::SetHeaderFile(const char* header) {
    if (mHeaderFileName != header) {
        if (nullptr != mHeaderFile) {
            fclose(mHeaderFile);
            mHeaderFile = nullptr;

            // propagate the change to the generators
            mHeader.SetOutputFile(nullptr);
        }
...
void XMLPacketGen::SetSourceFile(const char* source) {
    if (mSourceFileName != source) {
        if (nullptr != mSourceFile) {
            fclose(mSourceFile);
            mSourceFile = nullptr;

            // propagate the change to the generators
            mClone.SetOutputFile(nullptr);
            mConstruct.SetOutputFile(nullptr);
            mDecode.SetOutputFile(nullptr);
            mDestruct.SetOutputFile(nullptr);
            mDump.SetOutputFile(nullptr);
            mEncode.SetOutputFile(nullptr);
        }
...
bool XMLPacketGen::OpenFiles() {
    ...
    if (!mSourceFileName.empty() && (nullptr == mSourceFile)) {
