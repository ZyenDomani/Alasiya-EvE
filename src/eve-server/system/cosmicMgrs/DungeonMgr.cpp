...
void DungeonMgr::AddDecoToVector(uint8 dunType, uint32 templateID, std::vector<uint16>& groupVec)
{
    ...
    // =====================================================================
    ESpawnPattern selectedPattern = Pattern_AreaWeighted; // Default

    using namespace Dungeon::Type;
    if (dunType == Anomaly || dunType == Rated) {
        selectedPattern = Pattern_DonutShell; // Keep combat center clear
    } else if (dunType == Magnetometric || dunType == Radar) {
        selectedPattern = Pattern_DebrisField; // Concentrated ruins clouds
    }

    // Static anchors for Debris Field generation (picks a new spot per dungeon initialization)
    // Note: In production, you might want to reset these at the start of CreateDeco instead.
    static double cloudCenterX = (MakeRandomUInt(20000, 40000)) * (MakeRandomUInt() % 2 == 0 ? 1 : -1);
    static double cloudCenterY = (MakeRandomUInt(20000, 40000)) * (MakeRandomUInt() % 2 == 0 ? 1 : -1);
    // Static vectors to hold multiple distinct debris centers for the whole dungeon
    static std::vector<GpsCoord> clusterAnchors;
    const uint8 maxClusters = 3; // Number of distinct debris fields far away

    // Initialize the distant fields once per dungeon build
    if (clusterAnchors.empty() && selectedPattern == Pattern_DebrisField) {
        for (uint8 c = 0; c < maxClusters; ++c) {
            // Smoothly split angles so clusters don't spawn on top of each other
            double clusterTheta = (EvE::Trig::Pi2 / maxClusters) * c + ((MakeRandomUInt(0, 100) / 100.0) * 0.5);

            // Push them way out into the background (e.g., 50km to 75km)
            double clusterRadius = MakeRandomUInt(50000, 75000);

            GpsCoord anchor;
            anchor.x = clusterRadius * cos(clusterTheta);
            anchor.z = clusterRadius * sin(clusterTheta);
            anchor.y = MakeRandomInt(-5000, 5000); // Varied vertical plane per field
            clusterAnchors.push_back(anchor);
        }
    }
    ...
    for (auto &cur : groupVec) {
        ...
        double chosenGridSize = BOX_SIZES[2]; // 240m default

        // Your Grid Layout Optimization Matrix
        switch (abbRootCategory) {
            case 1: chosenGridSize = BOX_SIZES[4]; break; // 3,840m
            case 3: chosenGridSize = BOX_SIZES[2]; break; // 240m
            case 4: chosenGridSize = BOX_SIZES[3]; break; // 960m
            case 6:
                if (cur == 670 || cur == 160) {
                    chosenGridSize = BOX_SIZES[5]; // 15,360m
                } else {
                    chosenGridSize = BOX_SIZES[3]; // 960m
                }
                break;
            default: chosenGridSize = BOX_SIZES[2]; break;
        }
        ...
        for (uint8 i = 0; i < origLevel; ++i) {
            ...
            // Smoothly distribute angles across 360 degrees instead of a hard limit
            double theta = (EvE::Trig::Pi2 / static_cast<double>(origLevel)) * i;

            // Add a slight structural variance per item iteration
            double angleJitter = (static_cast<double>(MakeRandomUInt(0, 100)) / 100.0) * 0.2;
            theta += angleJitter;
            double rawX = 0.0;
            double rawZ = 0.0; // Use Z for the horizontal plane math
            double rawY = 0.0;
            double headingAngle = 0.0; // Track orientation angle
            double maxDistance = (i + 1) * chosenGridSize;

            // =====================================================================
            // PROCEDURAL GEOMETRY DISTRIBUTION GENERATION (X and Z plane)
            // =====================================================================
            switch (selectedPattern) {
                case Pattern_AreaWeighted: {
                    double randomFactor = static_cast<double>(MakeRandomUInt(0, 1000)) / 1000.0;
                    double dynamicRadius = radius + (maxDistance * std::sqrt(randomFactor));
                    rawX = dynamicRadius * cos(theta);
                    rawZ = dynamicRadius * sin(theta);
                    break;
                }
                case Pattern_DonutShell: {
                    double minRing = maxDistance * 0.35;
                    double randomFactor = static_cast<double>(MakeRandomUInt(0, 1000)) / 1000.0;
                    double dynamicRadius = radius + minRing + ((maxDistance - minRing) * randomFactor);
                    rawX = dynamicRadius * cos(theta);
                    rawZ = dynamicRadius * sin(theta);
                    break;
                }
                case Pattern_DebrisField: {
                    // Assign items evenly across our far-away clusters
                    uint8 assignedCluster = i % clusterAnchors.size();
                    GpsCoord targetAnchor = clusterAnchors[assignedCluster];

                    // Generate a local spread around that specific cluster middle
                    double localSpreadTheta = (EvE::Trig::Pi2 / static_cast<double>(origLevel)) * i;
                    double localRadius = radius + (MakeRandomUInt(0, 1000) / 1000.0) * (maxDistance * 0.3);

                    rawX = targetAnchor.x + (localRadius * cos(localSpreadTheta));
                    rawZ = targetAnchor.z + (localRadius * sin(localSpreadTheta));
                    rawY = targetAnchor.y + MakeRandomInt(-2000, 2000);

                    // MATHEMATICAL ORIENTATION: Find the angle facing back to its cluster center
                    // EVE client typically expects radians or quaternions for rotations.
                    // atan2(dx, dz) gives the horizontal rotation angle pointing to the middle.
                    headingAngle = std::atan2(targetAnchor.x - rawX, targetAnchor.z - rawZ);
                    break;
                }
            }

            // Subtle vertical padding using Y for altitude (+y is up)
            //rawY = localIt->second.y + MakeRandomInt(-1200, 1200);

            // =====================================================================
            // SNAP TO SECTOR GRID (X and Z are horizontal, Y is vertical)
            // =====================================================================
