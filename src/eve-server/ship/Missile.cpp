...
Missile::Missile( SystemManager* system,
        ...
        m_lifeTimer(0)
{
    ...
    Destiny()->SetPosition(position, false);

    m_hitTimer.Disable();
    double flightTime = self->GetAttribute(AttrExplosionDelay).get_float();
    flightTime *= (1 + ( 0.1 * (ship->GetOperator()->GetChar()->GetSkillLevel(skillMissileBombardment, true)))); // 10% increase in flightTime
...
void Missile::ForcedSetPosition(const GPoint &pt) {
    Destiny()->SetPosition(pt, false);
}

void Missile::_HitTarget() {

    // Create Damage action:
    // This still needs ship, module, and implant bonuses
     ...
     */
    double Sr = m_target->Item()->GetAttribute(AttrSignatureRadius).get_float();    // this is a default number, based on itemtype
    ...
    double dAdjust = Min(v1, v2);

    // damage adjustments here...
    dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillWarheadUpgrades, true)))); // 5% increase in damage (upped from 2%)
    ...
    switch (m_self->groupID()) {
        ...
        case EVEDB::invGroups::Advanced_Light_Missile:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillLightMissiles, true)))); // 5% increase in damage
            if (m_self->groupID() == EVEDB::invGroups::Advanced_Light_Missile)
                dAdjust *= (1 + ( 0.03 * (pChar->GetSkillLevel(skillLightMissileSpecialization, true)))); // 3% increase in damage
            break;
        case EVEDB::invGroups::Heavy_Missile:
        case EVEDB::invGroups::FoF_Heavy_Missile:
        case EVEDB::invGroups::Advanced_Heavy_Missile:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillHeavyMissiles, true)))); // 5% increase in damage
            if (m_self->groupID() == EVEDB::invGroups::Advanced_Heavy_Missile)
                dAdjust *= (1 + ( 0.03 * (pChar->GetSkillLevel(skillHeavyMissileSpecialization, true)))); // 3% increase in damage
            break;
        case EVEDB::invGroups::Cruise_Missile:
        case EVEDB::invGroups::FoF_Cruise_Missile:
        case EVEDB::invGroups::Advanced_Cruise_Missile:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillCruiseMissiles, true)))); // 5% increase in
            if (m_self->groupID() == EVEDB::invGroups::Advanced_Cruise_Missile)
                dAdjust *= (1 + ( 0.03 * (pChar->GetSkillLevel(skillCruiseMissileSpecialization, true)))); // 3% increase in damage
            break;
        case EVEDB::invGroups::Torpedo:
        case EVEDB::invGroups::Advanced_Torpedo:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillTorpedoes, true)))); // 5% increase in damage
            if (m_self->groupID() == EVEDB::invGroups::Advanced_Torpedo)
                dAdjust *= (1 + ( 0.03 * (pChar->GetSkillLevel(skillTorpedoSpecialization, true)))); // 3% increase in damage
            break;
        case EVEDB::invGroups::Rocket:
        case EVEDB::invGroups::Advanced_Rocket:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillRockets, true)))); // 5% increase in damage
            if (m_self->groupID() == EVEDB::invGroups::Advanced_Rocket)
                dAdjust *= (1 + ( 0.03 * (pChar->GetSkillLevel(skillRocketSpecialization, true)))); // 3% increase in damage
            break;
        case EVEDB::invGroups::Defender_Missile:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillDefenderMissiles, true)))); // 5% increase in damage
            break;
        case EVEDB::invGroups::Assault_Missile:
        case EVEDB::invGroups::Advanced_Assault_Missile:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillHeavyAssaultMissiles, true)))); // 5% increase in damage
            if (m_self->groupID() == EVEDB::invGroups::Advanced_Assault_Missile)
                dAdjust *= (1 + ( 0.03 * (pChar->GetSkillLevel(skillHeavyAssaultMissileSpecialization, true)))); // 3% increase in damage
            break;
        case EVEDB::invGroups::Citadel_Cruise:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillCitadelCruiseMissiles, true)))); // 5% increase in damage
            break;
        case EVEDB::invGroups::Citadel_Torpedo:
            dAdjust *= (1 + ( 0.05 * (pChar->GetSkillLevel(skillCitadelTorpedoes, true)))); // 5% increase in damage
            break;
    }

    if (IsOverloaded())
        dAdjust *= (1 + m_self->GetAttribute(AttrOverloadDamageModifier).get_float());
...
void Missile::EncodeDestiny( Buffer& into ) const {
    const GPoint& position = GetPosition();

    Destiny::BallHeader head;
    head.entityID = GetID();
    head.mode = Destiny::DSTBALL_MISSILE;
    head.radius = /*GetRadius*/ 5.0f;
    head.x = position.x;
    head.y = position.y;
    head.z = position.z;
    head.flags = Destiny::IsFree;
    into.Append( head );
...
PyDict* Missile::MakeSlimItem() const {
    ...
    slim->SetItemString("numLaunchers",     new PyInt(1));  //TODO fix this
    slim->SetItemString("nameID",           new PyInt(0));  //TODO fix this
