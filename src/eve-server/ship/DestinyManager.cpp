...
void DestinyManager::SetShipVariables(InventoryItemRef ship)
{
    ...
    m_shipInertia = ship->GetDefaultAttribute(AttrInetiaMod).get_float();
    if (!m_shipInertia)
        m_shipInertia = 1.0;
...
void DestinyManager::SetShipCapabilities(InventoryItemRef ship, bool undock)
{
    ...
    ship->SetAttribute(AttrInetiaMod, m_shipInertia);
...
void DestinyManager::MakeMissile(Missile* pMissile) {
    ...
    m_shipInertia = pMissile->GetSelf()->GetAttribute(AttrInetiaMod).get_float();
    m_shipAgility = m_massMKg * m_shipInertia;
