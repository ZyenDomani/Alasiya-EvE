...
static bool translocate_to_solarsystem(TRData *data, uint32_t who, uint32_t dest) {
    ...
    if (client->GetShipSE() and client->GetShipSE()->DestinyMgr() and !client->GetShipSE()->DestinyMgr()->IsCloaked()) {
...
static bool translocate_to_locationID(TRData *data, uint32_t who, uint32_t dest) {
}
...
bool translocate_to(TRData *data, uint32_t who, uint32_t dest, LocationTag tag) {
    ...
    case LocationTag_Character: {
        translocate_to_characterID(data, who, dest);
    } break;
    case LocationTag_SolarSystem: {
        translocate_to_solarsystem(data, who, dest);
    } break;
    case LocationTag_Celestial: {
        translocate_to_celestial(data, who, dest);
    } break;
    case LocationTag_Station: {
        translocate_to_station(data, who, dest);
    } break;
