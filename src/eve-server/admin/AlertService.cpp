...
PyResult AlertService::Handle_BeanCount(PyCallArgs &call) {
    _log(CLIENT__WARNING, "AlertService::Handle_BeanCount(): size=%lu", call.tuple->size() );
    ...
    result->items[1] = PyStatic.NewZero();        //loggingMode, 0=local, 1=DB (Capt: This isn't correct at all as it seems..)
...
{
    _log(CLIENT__WARNING, "AlertService::Handle_BeanDelivery(): size=%lu", call.tuple->size() );
    //call.Dump(CLIENT__CALL_DUMP);
...
{
    _log(CLIENT__WARNING, "AlertService::Handle_GroupBeanDelivery(): size=%lu", call.tuple->size() );
    //call.Dump(CLIENT__CALL_DUMP);
...
PyResult AlertService::Handle_SendClientStackTraceAlert(PyCallArgs &call) {
    _log(CLIENT__WARNING, "AlertService::Handle_SendClientStackTraceAlert(): size=%lu", call.tuple->size() );
    //call.Dump(CLIENT__CALL_DUMP);
