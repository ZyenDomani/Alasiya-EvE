...
m_dispatch(new Dispatcher(this))
{
    ...
    PyCallable_REG_CALL(ContractProxy, PlaceBid);
    PyCallable_REG_CALL(ContractProxy, FinishAuction);
    PyCallable_REG_CALL(ContractProxy, SplitStack);
    PyCallable_REG_CALL(ContractProxy, GetItemsInContainer);
    PyCallable_REG_CALL(ContractProxy, DeleteNotification);
    PyCallable_REG_CALL(ContractProxy, GetCourierContractFromItemID);
...
PyResult ContractProxy::Handle_SearchContracts(PyCallArgs &call) {
    ...
    if (!call.byname.find("contractType")->second->IsNone()) {
        ...
        PyList* contracts = new PyList(); //ContractUtils::GetContractEntries(contractIDs);
...
/*
    ...
    // Then, we go for actual entries creation.   Contract entry
...
PyResult ContractProxy::Handle_GetContract(PyCallArgs &call) {
    ...
    return nullptr; //ContractUtils::GetContractEntry(contractID->value());
}
...
PyResult ContractProxy::Handle_AcceptContract(PyCallArgs &call) {
    ...
    if (status == 0) {
        ...
        int64 timestamp = GetFileTimeNow();
        switch (contractType) {
            case 1: {
                // Item Exchange
    ...
    switch (completionStatus) {
        ...
        case 7: {
            ...
            if (collateral > 0) {
                ...
                call.client->AddBalance(collateral);
                AccountService::TransferFunds(call.client->GetCharacterID(), issuerID, collateral, "Collateral payment for failed contract", Journal::EntryType::ContractCollateral, contractID);
            }
...
PyResult ContractProxy::Handle_GetItemsInStation(PyCallArgs &call) {
    int stationID = call.tuple->GetItem(0)->AsInt()->value();

    if (!sDataMgr.IsStation(stationID))
        return nullptr;
// fix this
    return sItemFactory.GetStationRef(stationID)->GetMyInventory()->List(flagHangar);
}
