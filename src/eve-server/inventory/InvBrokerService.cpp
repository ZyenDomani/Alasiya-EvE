...
PyBoundObject* InvBrokerService::CreateBoundObject(Client* pClient, const PyRep *bind_args) {
    ...
    // TODO:  i think there is more to this....OID+ maybe?
    return new InvBrokerBound(m_manager, args.locationID, args.groupID);
}

//TODO:  is this completely right?  on quick glance, not really
PyResult InvBrokerBound::Handle_GetContainerContents(PyCallArgs &call)
{
    ...
    if (iRef->ownerID() == call.client->GetCharacterID()) {
        _log(INV__MESSAGE, "Handle_GetContainerContents() -  %s(%u) is owned by calling character %s(%u) ", \
                    iRef->name(), iRef->itemID(), call.client->GetName(), call.client->GetCharacterID());
    } else if (sDataMgr.IsSolarSystem(args.arg2)) {
...
PyResult InvBrokerBound::Handle_GetInventoryFromId(PyCallArgs &call) {
    ...
    _log(INV__DUMP, "InvBrokerBound::Handle_GetInventoryFromId() - size=%zu", call.tuple->size());
    call.Dump(INV__DUMP);
    }
    ...
    bool passive(args.arg2 != 0);  //no idea what this is for...update: to load or not
...
PyResult InvBrokerBound::Handle_GetInventory(PyCallArgs &call) {
    ...
    if (is_log_enabled(INV__DUMP)) {
    _log(INV__DUMP, "InvBrokerBound::Handle_GetInventory() size=%zu", call.tuple->size());
    call.Dump(INV__DUMP);
    }
...
PyResult InvBrokerBound::Handle_BreakPlasticWrap(PyCallArgs &call) {
    ...
    sLog.Warning("InvBrokerBound::Handle_BreakPlasticWrap()", "size= %zu", call.tuple->size());
    if (is_log_enabled(INV__DUMP))
    call.Dump(INV__DUMP);
...
PyResult InvBrokerBound::Handle_TakeOutTrash(PyCallArgs &call) {
    ...
    sLog.Warning("InvBrokerBound::Handle_TakeOutTrash()", "size= %zu", call.tuple->size());
    if (is_log_enabled(INV__DUMP))
    call.Dump(INV__DUMP);
...
PyResult InvBrokerBound::Handle_DeliverToCorpHangar(PyCallArgs &call) {
    ...
    */
    ...
    sLog.Warning("InvBrokerBound::Handle_DeliverToCorpHangar()", "size= %zu", call.tuple->size());
    if (is_log_enabled(INV__DUMP))
    call.Dump(INV__DUMP);
...
PyResult InvBrokerBound::Handle_DeliverToCorpMember(PyCallArgs &call) {
    ...
    */
    sLog.Warning("InvBrokerBound::Handle_DeliverToCorpMember()", "size= %zu", call.tuple->size());
    if (is_log_enabled(INV__DUMP))
    call.Dump(INV__DUMP);
