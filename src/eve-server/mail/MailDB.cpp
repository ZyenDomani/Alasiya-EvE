...
int MailDB::SendMail(int sender, std::vector<int>& toCharacterIDs, int toListID, int toCorpOrAllianceID, std::string& title, std::string& body, int isReplyTo, int isForwardedFrom)
{
    ...
    if (!sDatabase.RunQueryLID(err, messageID,
                               ...
                               bodyEscaped.c_str(), GetFileTimeNow()))
    {
...
PyObject* MailDB::GetNotificationsByGroup(int32 characterID, int32 groupID) {
    ...
    if (groupID == Notify::Groups::Old) {
        ...
        if (!sDatabase.RunQuery(res, queryStr.c_str(), characterID))
            return nullptr;
    } else {
        queryStr = "SELECT notificationID, typeID, senderID, receiverID, processed, created, data, deleted "
        "FROM eveNotifications WHERE receiverID = %i AND typeID IN (%s) AND deleted = 0";
        if (!sDatabase.RunQuery(res, queryStr.c_str(), characterID, typeFilter.c_str()))
            return nullptr;
    }
