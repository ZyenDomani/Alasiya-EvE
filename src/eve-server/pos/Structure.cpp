...
{
    _log(ITEM__TRACE, "Created Structure object for item %s (%u).", itemName().c_str(), itemID());
    ...
    for (int16 i; i<miniballsCount; i++) {
        ...
        miniball.clear();
                                    ...
                                    [MiniBall]
                                      ...
                                      */
    _log(COMMON__WARNING, "StructureEntity::EncodeDestiny(): %s - id:%u, mode:%u, flags:0x%X", GetName(), head.entityID, head.mode, head.flags);
}
...
void StructureEntity::MakeDamageState(DoDestinyDamageState &into) const
{
    ...
    into.recharge = m_self->GetAttribute(AttrShieldRechargeRate).get_float() +9;
...
PyDict *StructureEntity::MakeSlimItem() const {
    _log(COMMON__WARNING, "MakeSlimItem for StructureEntity %u", Item()->itemID());
