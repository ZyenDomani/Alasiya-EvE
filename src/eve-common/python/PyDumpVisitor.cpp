...
{
    _print( "%s Buffer (%zu):", _pfx(), rep->content().size() );
...
bool PyDumpVisitor::VisitString( const PyString *rep )
{
    if (IsPrintable(rep)) {
        ...
        _print( "%sBinary String: (len=%zu)", _pfx(), rep->content().length() );
    }
...
bool PyDumpVisitor::VisitWString( const PyWString* rep )
{
    ...
    if (IsPrintable(rep)) {
        ...
        _print( "%ssBinary WString: (len=%zu)'", _pfx(), rep->content().length() );
    }
...
bool PyDumpVisitor::VisitTuple( const PyTuple* rep )
{
    ...
    if (rep->empty()) {
        ...
        _print( "%s Tuple: %zu elements", _pfx(), rep->size() );
...
bool PyDumpVisitor::VisitList( const PyList* rep )
{
    ...
    if (rep->empty()) {
        ...
        _print( "%s  List: %zu elements", _pfx(), rep->size() );
...
bool PyDumpVisitor::VisitDict( const PyDict* rep )
{
    if (rep->empty()) {
        ...
        _print( "%s Dictionary: %zu entries", _pfx(), rep->size() );
