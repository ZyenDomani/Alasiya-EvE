...
uint32 DumpBall(LogType into, const uint8 *data, uint32 len) {
    ...
    _log(into, "AddBall: entity=%d, mode=%s (%d) flags=0x%X",
        ballhead->entityID, Destiny::DSTBALL_modeNames[ballhead->mode], ballhead->mode, ballhead->flags);
    ...
    //not right:
    if( 0 /*( ballhead->flags == 64 ) || ( ballhead->flags == 66 )*/ )
    {
